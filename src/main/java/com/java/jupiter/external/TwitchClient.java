package com.java.jupiter.external;
...
/**
 * Twitch client act as the Twitch API HTTP Client Object used to request resources from twitch API.
 * Supported resources: Stream, video, clips
...
//    private static final String CLIENT_ID;
    private static final String TOKEN = "Bearer 44zb775s55hejtxb2itkq94jtjvpok";
    private static final String CLIENT_ID = "z45hadrgbuxk3d6tpctuyevdnr1io0";
...
//            InputStream input = TwitchClient.class.getClassLoader().getResourceAsStream("config.properties");
//            if (input == null) {
//                System.out.println("Unable to find config.properties");
//            }
//            //load a properties file from class path, inside static method
//            prop.load(input);
//            tmpToken = prop.getProperty("db.url");
//            tmpClientId = prop.getProperty("db.user");
//        } catch (IOException e){
...
//    }
    ...
    // Build the request URL using GameName which will be used when calling Twitch APIs,
    ...
    // Build the request URL using GameId which will be used when calling Twitch APIs,
    ...
    // Send HTTP request to Twitch Backend based on the given URL(configured), and returns the body of the HTTP response returned from Twitch backend.
    ...
    // The Main Function: Integrate search() and getGameList() together, returns the top x popular games from Twitch.
    ...
    // The Main Function: Integrate search() and getGameList() together, returns the dedicated game based on the game name.
    public Game searchGame(String gameName) throws TwitchException {
        List<Game> gameList = getGameList(searchTwitch(buildGameURL(GAME_SEARCH_URL_TEMPLATE, gameName, 0)));
        //only return the first Game from the list
        if (gameList.size() != 0) {
    ...
    private List<Item> searchStreams(String gameId, int limit) {
        ...
        for (Item item : streams) {
            //Twitch does not return the type and url for Stream, but the fields are preferred in the Item object,
            //so we manually set them for more informative result.
    ...
    private List<Item> searchClips(String gameId, int limit) {
        ...
        //Twitch does not return the type for Clip, but the fields are preferred in the Item object,
        //so we manually set them for more informative result.
        for (Item item : clips) {
    ...
    private List<Item> searchVideos(String gameId, int limit) throws TwitchException {
        ...
        //Twitch does not return the type for Video, but the fields are preferred in the Item object,
        //so we manually set them for more informative result.
    ...
    //Integrates searchStreams(), searchVideos() and  searchClips() to enable using GameId and Type to search resources
    ...
    //Main function: used to search items of all types, returns a Map of <ItemType: List<Item>>
